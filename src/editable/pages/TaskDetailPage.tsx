    ...
    <section className="bg-white">
      <header>
        <div className="mx-auto max-w-[1180px] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <BackLink task={task} />
          <div className="mt-10 flex flex-wrap items-center gap-3 text-sm font-black">
            <span className="rounded-full bg-[var(--slot4-accent-soft)] px-4 py-2 text-[var(--slot4-accent)]">{categoryOf(post, 'News')}</span>
            <span className="rounded-full border border-[var(--slot4-border)] px-4 py-2 text-[var(--slot4-muted-text)]">Media distribution</span>
          </div>
          <h1 className="mt-7 max-w-6xl text-5xl font-extrabold leading-tight text-[#090821] sm:text-6xl lg:text-[5rem]">{post.title}</h1>
        </div>
      </header>

      {images[0] ? (
        <figure className="mx-auto max-w-[1180px] overflow-hidden rounded-[1.1rem] border border-[var(--slot4-border)] bg-white shadow-[0_22px_80px_rgba(15,23,42,.08)]">
      ...
      <div className="mx-auto grid max-w-[1180px] gap-12 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,760px)_300px] lg:px-8 lg:py-16">
        <article className="min-w-0 rounded-[1.1rem] border border-[var(--slot4-border)] bg-white p-6 shadow-[0_18px_60px_rgba(15,23,42,.06)] sm:p-8">
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[1.1rem] border border-[var(--slot4-border)] bg-white p-5 shadow-sm">
          <p className="text-xs font-black uppercase tracking-[0.18em] text-[var(--slot4-accent)]">Post details</p>
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-[var(--slot4-border)] py-3 transition hover:text-[var(--slot4-accent)]">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-20 w-20 shrink-0 rounded-[1rem] object-cover" /> : <div className="flex h-20 w-20 shrink-0 items-center justify-center rounded-[1rem] bg-[#0c0a24] text-white"><FileText className="h-6 w-6" /></div>}
